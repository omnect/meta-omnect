...
do_install_append_rpi() {
  sed -i 's/^After=\(.*\)$/After=\1 dev-tpmrm0.device time-sync.target/' ${D}${systemd_system_unitdir}/adu-agent.service
}
...
USERADD_PARAM_${PN} = "--no-create-home -r -s /bin/false -g adu -G disk,tpm adu"
